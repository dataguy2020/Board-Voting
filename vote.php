    ...
    foreach ($motionArray as $motion) {
        ...
        while ($row = $motion->fetch(PDO::FETCH_ASSOC)) {
            ...
            $body .= "<h1>" . $motionname . "</h1>
                                       ...
                                       <h2>Motion Text</h2>" . $motiondesc . "<h2>Disposition:</h2>" . $disposition;
        } //End of while
        $body .= "<br /><br />
                    <h2>Current Votes</h2>
        ...
        $votes = $db_con->prepare("SELECT u.first_name, u.last_name, v.time, v.vote from votes v 
        ...
        $motiondiscussions = $db_con->prepare("SELECT u.first_name,u.last_name,d.dateadded,d.discussion_text
        ...
        $changeLog = $db_con->prepare("SELECT u.first_name,u.last_name,mcl.date, mcl.field,mcl.oldValue,mcl.newValue 
                    FROM users u 
...
?>
           <br /><br />
            ...
            <?php
    $votes = $db_con->prepare("SELECT u.first_name, u.last_name, v.time, v.vote from votes v inner join motions m on m.motion_id=v.motions_id INNER join users u on u.users_id=v.users_id where m.motion_id=:motionid");
    $votes->bindParam(':motionid', $motionid);
    $votes->execute();
    while ($row = $votes->fetch(PDO::FETCH_ASSOC)) {
        $firstname = $row['first_name'];
...
?>
        ...
        <?php
    $motiondiscussions = $db_con->prepare("SELECT u.first_name,u.last_name,d.dateadded,d.discussion_text from users u inner join discussion d on d.user_id=u.users_id where d.motion_id=:motionid");
    $motiondiscussions->bindParam(':motionid', $motionid);
    $motiondiscussions->execute();
    while ($row = $motiondiscussions->fetch(PDO::FETCH_ASSOC)) {
        $firstname      = $row['first_name'];
...
?>
	...
	<form id="voting" name="voting" method="POST" action="amendMotion.php">
        ...
        <input type="hidden" name="existingmotiondec" id="existingmotiondec" value="' . htmlspecialchars($motiondesc) . '">
